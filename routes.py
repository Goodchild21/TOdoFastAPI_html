...
@app.post('/add')
def add(title: str = Form(...), db_session: Session = Depends(get_db)): #в теге title name="title", ... обязательное поле
    new_todo = ToDo(title=title)
    db_session.add(new_todo)
    db_session.commit()

    url = app.url_path_for('home')#Вернуться в home после создания записи
...
def update(todo_id: int, db_session: Session = Depends(get_db)):
    ...
    todo.is_complete = True #not todo.is_complete
    db_session.commit()
